...
# (1)、在python中，类型是动态的, 默认情况下运行是不会进行类型检查的, BaseModel就是用于做运行时类型检查的类, 只需让类继承BaseModel就可以进行运行检查
# (2)、在python中，类型是动态的, 默认情况下语法服务器时不会进行类型检查的，TypeDict就是用于语法服务器做类型检查的, 只需让类继承TypeDict就可以进行语法服务器检查
