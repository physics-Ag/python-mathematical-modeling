...
print(A)
print(B)
print(type(A))      # 注：我们口头上称呼A是矩阵，但是我们实际上要知道A的类型是<class 'numpy.ndarray'>，这是NumPy内部自定义的数据结构
...
# 3、矩阵的dtype
# dtype是用于说明矩阵用什么类型来存储数据，可以自己设置，它是NumPy内部自定义的类型，详见dtype.png => 这也间接说明了矩阵存储的元素类型是相同的
print(B.dtype)
